...
void CMdSpi::SubscribeMarketData(char ** id, int num)
{
	int iResult0 = pMdApi->UnSubscribeMarketData(id, num);
	cerr << "--->>> 取消行情订阅请求: " << ((iResult0 == 0) ? "成功" : "失败") << endl;
	Sleep(1000);
	int iResult = pMdApi->SubscribeMarketData(id, num);
...
void CMdSpi::OnRspSubMarketData(CThostFtdcSpecificInstrumentField *pSpecificInstrument, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	...
	CString str(pSpecificInstrument->InstrumentID);
	str = str + CString("成功订阅!");
	theApp.Message(str);
...
{
	theApp.OnRtnDepthMarketData(pDepthMarketData);
}
void CMdSpi::OnRtnDepthMarketData1(CThostFtdcDepthMarketDataField *pDepthMarketData)	
{
	//cerr << "--->>> " << __FUNCTION__ << endl;
	///数据缓存。
	ReceiveTick = true;
